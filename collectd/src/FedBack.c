...
static void ps_list_register (const char *name, const char *regexp)
{
	...
	if (new == NULL)
	{
		ERROR ("FedBack plugin: ps_list_register: malloc failed.");
		return;
	}
	...
	if (regexp != NULL)
	{
		ERROR ("FedBack plugin: ps_list_register: "
...
static int ps_config (oconfig_item_t *ci)
{
	...
	for (i = 0; i < ci->children_num; ++i) {
		...
		else if (strcasecmp (c->key, "ProcessMatch") == 0)
		{
			...
			{
				ERROR ("FedBack plugin: `ProcessMatch' needs exactly "
			...
			if (c->children_num != 0) {
				WARNING ("FedBack plugin: the `ProcessMatch' config option "
						"does not expect any child elements -- ignoring "
...
int ps_read_process (int pid, procstat_t *ps, char *state)
{
	...
	{
		ERROR ("FedBack plugin: name_start_pos = %zu >= name_end_pos = %zu",
	...
	*state = fields[0][0];
	...
	if (ps->num_proc == 0)
	{
		DEBUG ("FedBack plugin: This is only a zombi: pid = %i; "
				"name = %s;", pid, ps->name);
		return (0);
	}
...
static char *ps_get_cmdline (pid_t pid, char *name, char *buf, size_t buf_len)
{
	...
	if (fd < 0) {
		...
		if (errno != ENOENT)
			WARNING ("FedBack plugin: Failed to open `%s': %s.", file,
					sstrerror (errno, errbuf, sizeof (errbuf)));
		return NULL;
	}
	...
	while (42) {
		...
		if (status < 0) {
			...
			WARNING ("FedBack plugin: Failed to read from `%s': %s.", file,
					sstrerror (errno, errbuf, sizeof (errbuf)));
			close (fd);
			return NULL;
		}
...
static unsigned long read_fork_rate ()
{
	...
	if (proc_stat == NULL) {
		char errbuf[1024];
		ERROR ("FedBack plugin: fopen (/proc/stat) failed: %s",
				sstrerror (errno, errbuf, sizeof (errbuf)));
		return ULONG_MAX;
	}
	...
	while (fgets (buf, sizeof(buf), proc_stat) != NULL)
	{
		...
		if ((endptr == fields[1]) || (errno != 0)) {
			ERROR ("FedBack plugin: Cannot parse fork rate: %s",
					fields[1]);
			result = ULONG_MAX;
			break;
		}
...
/* #endif KERNEL_LINUX */
	...
	if (kd == NULL)
	{
		ERROR ("FedBack plugin: Cannot open kvm interface: %s",
				errbuf);
		return (0);
	}
	...
	if (procs == NULL)
	{
		ERROR ("FedBack plugin: Cannot get kvm processes list: %s",
				kvm_geterr(kd));
		kvm_close (kd);
		return (0);
	}
		...
		if ((proc_ptr == NULL) || (proc_ptr->ki_pid != procs[i].ki_pid))
		{
					...
					&& (procs[i].ki_args != NULL))
			{
				...
				if ((argv != NULL) && (argv[0] != NULL))
				{
					...
					if (status < 0)
						WARNING ("FedBack plugin: Command line did not fit into buffer.");
					else
						have_cmdline = 1;
				}
